...
public interface BarmanRepository extends JpaRepository<Barman, Long> {
    ...
    Optional<Barman> findByCocktailsId(Long cocktailId);
    ...
    @Query("""
            SELECT NEW io.takima.springdatabbl.model.projection.BarmanProjection(b.name AS name, c.name AS cocktailName) 
                FROM Barman b LEFT JOIN b.cocktails c WHERE b.id = :id""")
